...
def invoke_endpoint_streaming(
        ...
        try:
            ...
            # Ensure model_id is included if available in session state
            if "selected_model_id" in st.session_state and "model_id" not in body:
                body["model_id"] = st.session_state["selected_model_id"]
...
class ChatManager:
    ...
    def _init_session_state(self):
        ...
        if "agent_arn" not in st.session_state:
            agent_arn = get_ssm_parameter("/app/smartgoalgenerator/agentcore/runtime_arn")
    ...
    def process_user_message(self, prompt: str, actor_id: str, bearer_token: str):
        ...
        with st.chat_message("assistant"):
            ...
            create_safe_markdown_text(
                '<span class="thinking-bubble">🤖 💭 Smart goal generator is thinking...</span>',
                message_placeholder,
            )
            ...
            # Include model_id in payload if available
            payload_data = {"prompt": prompt, "actor_id": actor_id}
            if "selected_model_id" in st.session_state:
                payload_data["model_id"] = st.session_state["selected_model_id"]
